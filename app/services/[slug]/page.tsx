...
export default function ServiceInquiryPage() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formspreeEndpoint) {
        ...
        // Simulated instant success if Formspree ID is not yet customized
        console.log("LUCIAN Service Inquiry Submitted:", {
          serviceName: service.title,
          ...formData,
        });
      ...
      router.push(`/services/confirmation?service=${encodeURIComponent(service.title)}`);
    } catch (err) {
      console.error("Submission failed:", err);
      // Still allow navigation to confirmation after user notification, with fallback
      setSubmitError(
        "Oops! Direct email submission encountered an issue. Please try again or message us directly on WhatsApp."
      );
  ...
  const whatsappPreFilledUrl = `https://wa.me/9779818587406?text=${encodeURIComponent(
    `Hi LUCIAN, I'm interested in ${service.title}. Name: ${formData.fullName || "Prospective Client"}`
  )}`;
          ...
          <div className="p-8 sm:p-12 rounded-3xl bg-surface border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.8)] border-b-4 border-b-gold animate-in fade-in duration-300">
            <div className="flex items-start justify-between gap-4 mb-8">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-2xl bg-gold/10 border border-gold/30 text-gold flex items-center justify-center shadow-[0_0_20px_rgba(245,176,65,0.2)]">
              ...
              <a
                ...
                className="hidden sm:inline-flex items-center gap-2 text-xs font-mono text-gold hover:underline p-2 rounded-lg bg-gold/5 border border-gold/20"
              >
